...
export default function Match() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="stadium border border-line rounded-xl p-5 sm:p-8 md:p-10 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 items-center gap-4 relative z-10">
          <div className="text-center sm:text-right md:text-left">
            <div className="text-6xl sm:text-7xl md:text-9xl mb-1 sm:mb-2">{home.flag}</div>
            <div className="font-display font-extrabold text-xl sm:text-2xl md:text-3xl uppercase">{home.nameEn}</div>
            <div className="font-mono text-xs text-gold tracking-wider">ELO {Math.round(home.elo)}</div>
          </div>
          <div className="text-center order-first sm:order-none py-2 sm:py-0 border-y border-line/40 sm:border-0">
            <div className="kicker text-red mb-1 sm:mb-2">VS</div>
            <div className="font-display font-extrabold text-3xl sm:text-4xl md:text-6xl gold-fill">
              {stats!.homeXg.toFixed(2)} : {stats!.awayXg.toFixed(2)}
            </div>
            <div className="font-mono text-[10px] tracking-wider text-dim mt-1 sm:mt-2">EXPECTED GOALS</div>
            <div className="kicker text-[10px] mt-2 sm:mt-3">
              {isKnockout ? 'KNOCKOUT · 加时 + 点球' : 'GROUP STAGE'}
            </div>
          </div>
          <div className="text-center sm:text-left md:text-right">
            <div className="text-6xl sm:text-7xl md:text-9xl mb-1 sm:mb-2">{away.flag}</div>
            <div className="font-display font-extrabold text-xl sm:text-2xl md:text-3xl uppercase">{away.nameEn}</div>
      ...
      <section className="grid grid-cols-3 gap-2 sm:gap-4">
        <div className="panel border-l-[3px] border-gold/40 bg-gold-soft p-3 sm:p-5">
          <div className="kicker mb-1 sm:mb-2 text-[9px] sm:text-[11px] truncate">{home.nameEn} WIN</div>
          <div className="font-display font-extrabold text-2xl sm:text-4xl gold-fill">{pct(stats!.homeWin / total)}</div>
        </div>
        <div className="panel border-l-[3px] border-line p-3 sm:p-5">
          <div className="kicker mb-1 sm:mb-2 text-dim text-[9px] sm:text-[11px]">DRAW</div>
          <div className="font-display font-extrabold text-2xl sm:text-4xl text-white">
            {isKnockout ? '—' : pct(stats!.draw / total)}
          </div>
        </div>
        <div className="panel border-l-[3px] border-red/40 bg-red-soft p-3 sm:p-5">
          <div className="kicker mb-1 sm:mb-2 text-red text-[9px] sm:text-[11px] truncate">{away.nameEn} WIN</div>
          <div className="font-display font-extrabold text-2xl sm:text-4xl gold-fill">{pct(stats!.awayWin / total)}</div>
        </div>
      </section>

      <section className="panel p-4 sm:p-6">
        <div className="kicker mb-3">RESULT DISTRIBUTION</div>
        <h3 className="font-display font-extrabold text-xl sm:text-2xl uppercase mb-2">3-Way Probabilities</h3>
      ...
      <section className="panel p-4 sm:p-6">
        ...
        <h3 className="font-display font-extrabold text-xl sm:text-2xl uppercase mb-1">单场进球分布</h3>
      ...
      <section className="panel p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-2">
          {stats!.topScores.map((s, i) => (
            <div key={s.score} className="flex items-center gap-2 sm:gap-4 py-2 border-b border-line/50 last:border-0">
              <span className="font-display text-xl sm:text-2xl text-gold w-7 sm:w-8 text-center shrink-0">{(i + 1).toString().padStart(2, '0')}</span>
              <span className="font-display font-extrabold text-2xl sm:text-3xl gold-fill shrink-0">{s.score}</span>
              <div className="flex-1 min-w-0"><ProbBar value={s.count / 5000} /></div>
              <span className="font-mono text-xs sm:text-sm text-white tabular-nums w-12 sm:w-16 text-right shrink-0">
                {pct(s.count / 5000)}
              </span>
            </div>
          ))}
